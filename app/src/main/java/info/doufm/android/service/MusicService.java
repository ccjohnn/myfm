...
public class MusicService extends Service implements MediaPlayer.OnCompletionListener, MediaPlayer.OnErrorListener,
    ...
    public void onCreate() {
        ...
        if(myBinder == null){
            ...
            playMusicReceiver = new PlayMusicReceiver(myBinder,getApplicationContext());//从喜爱列表和历史列表里面发送过来的
            ...
            registerHeadSetListener();//这一段有什么用,没有发现广播的发送者，作用只是接受广播，关闭音乐
        }
    ...
    private void postNotification(String contentTitle, String contentText){
        ...
        //延迟将intent以广播发送出去，更新这个通知，当第二条到来时同时也更新第一条的值，除非他们的requestcode不同
    ...
    private void getUserLoveList(){
        if(User.getInstance().getLogin() && isFirstOpen){//这里其实每次开启service都进行下载喜欢列表
    ...
    public void playNextMusic(){
        ...
        if(myBinder.nextMusicInfo != null){
            ...
            if(myBinder.playMusicInfo.isHasCache()){//如果有这一首音乐的缓存就播放这一首音乐
                //播放缓存音乐
                myBinder.playCacheMusic();
                Log.v(TAG,"播放缓存音乐");
            }else{
                //播放在线音乐
                try {
                    myBinder.playRandomMusic();//如果没有缓存就请求播放
    ...
    public void playUserChooseMusic(MusicInfo musicInfo){
            pausePlayMusic();
    ...
    public void onCompletion(MediaPlayer mp) {
        uploadUserListenHistory();//上传听歌历史
        if(isPlaying == true){
            mediaPlayer.stop();
            isPlaying = false;
