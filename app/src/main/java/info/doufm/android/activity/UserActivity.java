...
public class UserActivity extends ActionBarActivity implements View.OnClickListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        themeNum = getIntent().getIntExtra(Constants.EXTRA_THEME, 11);
    ...
    private void getHistoryAndLoveNum() throws AuthFailureError {
        JsonObjectRequestWithCookie jsonObjectRequestWithCookie = new JsonObjectRequestWithCookie(
                NetworkConstans.USER_PROFILE_URL, null,new Response.Listener<JSONObject>() {
            @Override
        ...
        RequestManager.getRequestQueue(this).add(jsonObjectRequestWithCookie);
    }
    ...
    public void quit() throws AuthFailureError {
        JsonObjectRequestWithCookie jsonObjectPostRequestWithCookie = new JsonObjectRequestWithCookie(NetworkConstans.LOGOUT_URL, null, new Response.Listener<JSONObject>() {
            @Override
            public void onResponse(JSONObject jsonObject) {
        ...
        RequestManager.getRequestQueue(this).add(jsonObjectPostRequestWithCookie);
    }
