...
public class SplashActivity extends Activity {
    ...
    protected void onResume() {
        super.onResume();
        isNetworkAvailable(h, 2000);//检测网络是否可用
        MobclickAgent.onPageStart(TAG);
        MobclickAgent.onResume(this);//提供一些静态方法用于统计终端应用的用户行为常见的有在Resume和Pause过程中调用，MobclickAgent会记录用户进入了应用的哪个个页面
    }
        ...
        new Thread() {
            ...
            public void run() {
                ...
                new Thread() {
                    @Override
                    public void run() {
                       // HttpGet requestForTest = new HttpGet("http://doufm.info");过期了
                        try {
                            URL url = new URL("http://doufm.info");
